...
\begin{document}
	...
	\begin{abstract}
		...
		\normalsize{
			...
			\textbf{针对第二问}，我们构建了\textbf{引入了分时电价、购售电互斥和绿电指标的成本单一优化模型}，针对设备启停的两种状态，建立了以0-1为决策变量的\textbf{离散开关MILP模型}，分别对典型场景和对24种风光场景的5个产量下进行了优化求解。结果表明：典型场景下\textbf{最优日产量为36吨/日}，\textbf{设备利用率为50\%}；\textbf{全年加权平均吨氨成本为5943元/吨}，其中\textbf{39.17\%的场景满足全部绿电直连指标要求}。
			
			\textbf{针对第三问}，日产能72吨/日，电制氢氨装置功率连续可调。将设备由第二问的0-1开关升级为ALK、PEM、NH3独立拆分建模，引入\textbf{半连续功率约束}和\textbf{逐时氢气质量平衡约束}，建立\textbf{连续调节MILP模型}。对24种风光场景×5个产量级别共120个算例进行优化求解，得到每种场景下的最优制氨用电功率调度方案和相应绿电指标。全年统计显示：\textbf{绿电全满足场景占比由39.17\%提升至70.83\%}，\textbf{加权平均吨氨成本降低2.21\%}。
			
			\textbf{针对第四问}，切断电网连接后，在尽限利用风光发电条件下对24种场景进行无储能离网运行分析，发现最差场景\textbf{日产量仅6.25吨/天}，确认离网无储能不可行。并针对\textbf{W3S0}最大弃电场景建立含储能SOC动态方程和离网功率平衡约束的线性规划模型，优化得到\textbf{最优储能配置为容量186.36 MWh、功率34.2 MW}。将该配置推广至全部场景求解有储能调度方案，\textbf{全年制氨总量9512吨}，\textbf{吨氨成本5413元/吨}。与联网连续调节模式对比，\textbf{离网年产量为联网的73.4\%}，产能缺口明显。
			
			\textbf{针对第五问}，在前述连续调节模型基础上引入新能源渗透率参数，以风光容量缩放倍数为控制变量，求解不同渗透率水平下吨氨成本与运行指标的变化规律。结果表明：渗透率从0.27升至0.68时吨氨成本由8319元/吨降至6014元/吨，但外送电比例由0\%升至12.4\%且加速增长。渗透率超过0.55后系统主要矛盾由能源不足转变为新能源时序波动，过高装机比例在经济上不具优势。
			全参数敏感性分析表明设备容量参数对结果影响最显著，电解运维费次之，绿电惩罚系数影响有限。渗透率参数化分析揭示了低/中/高渗透率阶段的主要矛盾切换规律，为园区规划提供了定量依据。
			
		}
	\end{abstract}
	\vspace{0.5cm}
	
	\noindent \textbf{关键词：} 绿电直连；电氢氨园区；混合整数线性规划；单目标优化；离网储能配置
	...
	随着碳达峰、碳中和战略的持续推进，新能源大规模并网后的消纳问题日益突出，绿电直连模式为促进可再生能源就近消纳提供了有效路径。国家发展改革委、国家能源局联合发文明确要求绿电直连项目须同时满足新能源自发自用率、总用电量绿电比例和新能源上网电量比例三项强制性指标。在此背景下，“绿电—绿氢—绿氨”一体化耦合的化工园区成为解决新能源消纳与化工脱碳的重要方案。园区涵盖风力发电、光伏发电、碱性电解槽、质子交换膜电解槽、合成氨装置、常规电负荷及并网线路，以氨为最终产品外输。由于风光出力具有强随机波动性，需对园区内电解制氢与合成氨功率进行优化调度，在满足绿电指标的前提下实现吨氨成本最小化。
	...
	问题一要求设备满负荷连续运行，新能源出力由典型日风光曲线给定。在此条件下，园区总负荷（制氢氨设备加常规电负荷）在一天内基本恒定，而风光出力则随时间剧烈波动——午间光伏峰值与夜间零出力之间存在巨大落差。这意味着新能源供给与工业负荷之间存在\textbf{时间错配}：风光出力过剩时段产生余电需要外送，出力不足时段则必须从网购电弥补缺口。问题一的核心在于通过逐时功率平衡定量刻画这一错配程度，并评估其对绿电直连三项指标及吨氨成本的影响。
	...
	依据《关于有序推动绿电直连发展有关事项的通知》中规定的公式，计算三项核心指标：
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.70\textwidth]{q1_result.pdf}
	...
	设备状态因此不再是连续变量，而是典型的离散启停决策。不同时段风光出力和电价存在差异，需要在满足日产量的前提下决定哪些时段开机、哪些时段停机以实现运行成本最低。该问题本质上属于带时序耦合约束的混合整数优化问题，本文采用混合整数线性规划MILP建模。此外，同一时段购电与售电不可同时发生——若不加以约束，模型可能出现低价购电高价售电的非物理套利行为，因此引入购售电互斥约束。三项绿电指标若以硬约束形式直接施加，在部分低风光场景下可能导致模型无可行解，故采用软约束加经济惩罚的方式，在合规性与经济性之间寻求平衡。
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.85\textwidth]{q2_cost_distribution.pdf}
	...
	问题二采用离散启停方式，虽能通过选择运行时段在一定程度上适应新能源波动，但仍存在两个明显局限：其一，设备只能在全功率和零功率间切换，无法在风光出力较低但非零的时段按需降功率运行，导致可利用的绿电被浪费；其二，固定运行时长的约束（由产量除以额定产率确定）使设备利用率仅限于50\%--100\%的离散值，无法精细匹配连续变化的风光曲线。
	
	因此问题三将功率调节方式从离散升级为\textbf{连续可调}，允许设备在额定功率的10\%至100\%范围内任意调节。这一变化使功率变量从二值变量扩展为半连续变量，从而能够更精确地跟踪风光出力波动。
	
	\textbf{拆分设备独立建模的动机。}传统聚合模型将全部制氢负荷等效为单一设备，无法体现ALK与PEM在制氢效率（14 vs 16 kgH$_2$/(h$\cdot$MW)）和运维成本（0.10 vs 0.15 元/kWh）上的差异，也无法区分不同设备的最小稳定运行约束。本文将ALK、PEM和合成氨装置拆分独立建模，使优化器能够自主选择更经济的设备组合，实现\textbf{效率-成本协同优化}。
	...
	离网运行时园区功率平衡不含电网交换项，逐时新能源出力必须等于生产负荷与常规负荷之和：
	\begin{equation}
		P_{\text{wind}}(t) + P_{\text{solar}}(t) = p(t) + P_{\text{load}}(t) + E_{\text{curt}}(t), \quad \forall t
		\label{eq:q4_balance_ns}
	\end{equation}
	
	其中$p(t)$为制氢氨设备聚合功率，$E_{\text{curt}}(t)$为弃电功率。设备聚合功率受半连续运行区间约束，$p(t) \in \{0\} \cup [P_{\min}, P_{\text{total}}]$，下限$P_{\min}=4.15$ MW为各设备10\%最低功率之和，上限$P_{\text{total}}=41.5$ MW为满负荷总功率。给定逐时可利用风光出力，生产设备按尽限利用原则运行：当净功率$P_{\text{wind}}+P_{\text{solar}}-P_{\text{load}}$低于$P_{\min}$时设备停机，高于$P_{\text{total}}$时满负荷运行且余量弃电，介于二者之间时以净功率运行。
	
	对全部24种风光场景逐一核算，离网无储能的制氨产量结果如表\ref{tab:q4_nostorage}所示。日产氨量范围跨度极大，最差场景W1S3仅6.25吨/天，最优场景W3S0达46.19吨/天。全部24场景的年化总产量为9782吨，加权平均吨氨成本4057元/吨。
	
	\begin{table}[ht]
		\centering
		\small
		\caption{离网无储能24场景产量分布}
		\label{tab:q4_nostorage}
		\begin{tabular}{lccc}
			\toprule
			场景分类 & 场景数 & 日产量范围(吨/天) & 平均成本(元/吨) \\
			\midrule
			高风光资源(风电$\ge$ 3) & 12 & 16.68--46.19 & 4051 \\
			低风光资源(风电< 3) & 12 & 6.25--33.10 & 4203 \\
			全部场景 & 24 & 6.25--46.19 & 4057 \\
			\bottomrule
		\end{tabular}
	\end{table}
	
	无储能离网模式下，日产氨量完全取决于当日风光资源条件。低风速低光照场景下出力不足以维持设备最低负荷运行，导致产量骤降。离网无储能的年产量仅9782吨，远低于并网连续调节模式下的约12960吨，且无法保证稳定供货。这说明离网模式必须在配置储能缓冲的条件下才能具备工程可行性。
	
	\subsubsection*{5.4.2 储能优化配置}
	
	在无储能分析中，日弃电量最大的场景为W3S0，日弃电约173.7 MWh。该场景风光资源充足但时空错配严重——午间光伏过剩时电解槽已达满功率，余量被迫弃掉。储能系统可以吸收弃电并在出力不足时段释放，从而更充分地利用新能源电量。本节以W3S0场景为对象，构建含储能的优化模型求解最优储能容量与功率。
	
	\textbf{决策变量。}模型引入储能额定容量$C_{\text{bat}}$和额定功率$P_{\text{bat}}$作为优化变量，同时包含逐时充放电功率$P_c(t)$和$P_d(t)$、电池荷电状态$\text{SOC}(t)$、设备聚合功率$p(t)$、弃电功率$E_{\text{curt}}(t)$以及设备启停标志$z(t)$。
	
	\textbf{储能荷电状态动态方程。}储能的荷电状态随时间按式(\ref{eq:q4_soc})递推，充电效率$\eta_c=0.90$，放电效率$\eta_d=0.90$，自放电率$\sigma=0.002$：
	\begin{equation}
		\text{SOC}(t+1) = \text{SOC}(t) \cdot (1-\sigma) + \eta_c P_c(t) - \frac{P_d(t)}{\eta_d}
		\label{eq:q4_soc}
	\end{equation}
	
	\textbf{荷电状态边界约束。}为防止过充过放，SOC允许范围为额定容量的10\%至90\%。初始SOC设为50\%以保持对称调节能力，日末SOC必须回到初始值以保证日循环闭合：
	\begin{align}
		0.10 C_{\text{bat}} &\leq \text{SOC}(t) \leq 0.90 C_{\text{bat}} \quad \forall t \label{eq:q4_soc_bound} \\
		\text{SOC}(0) &= 0.50 C_{\text{bat}} \label{eq:q4_soc_init} \\
		\text{SOC}(24) &= \text{SOC}(0) \label{eq:q4_soc_term}
	\end{align}
	
	\textbf{充放电功率约束。}充放电功率受储能额定功率限制，且充放电不可同时进行——储能不能在同一时段既充电又放电，这一物理约束通过大M法实现：
	\begin{align}
		0 &\leq P_c(t) \leq P_{\text{bat}} \quad \forall t \label{eq:q4_pc} \\
		0 &\leq P_d(t) \leq P_{\text{bat}} \quad \forall t \label{eq:q4_pd} \\
		P_c(t) &\leq M \cdot (1 - y(t)), \quad P_d(t) \leq M \cdot y(t) \quad \forall t \label{eq:q4_mutex}
	\end{align}
	
	\textbf{功率平衡约束。}离网功率平衡在式(\ref{eq:q4_balance_ns})基础上增加储能充放电项：
	\begin{equation}
		P_{\text{wind}}(t) + P_{\text{solar}}(t) + P_d(t) = P_c(t) + p(t) + P_{\text{load}}(t) + E_{\text{curt}}(t), \quad \forall t
		\label{eq:q4_balance}
	\end{equation}
	
	电量充足时储能充电，不足时储能放电，储能参与实现了电能在时间维度的平移。
	
	\textbf{产量与半连续约束。}设备聚合功率$p(t)$仍满足半连续约束，日产量通过功率累加与合成氨效率折算实现：
	\begin{align}
		p(t) &\leq P_{\text{total}} \cdot z(t), \quad p(t) \geq P_{\min} \cdot z(t) \quad \forall t \label{eq:q4_semi} \\
		\sum_{t=0}^{23} p(t) \times \rho_{\text{NH3}} \times 1000 &\geq D_{\text{target}} \label{eq:q4_prod}
	\end{align}
	
	\textbf{储能倍率约束。}储能额定功率与额定容量满足4C倍率限制，即最大充放电功率不超过额定容量的4倍：
	\begin{equation}
		P_{\text{bat}} \leq 4 C_{\text{bat}}
		\label{eq:q4_crate}
	\end{equation}
	
	\textbf{目标函数。}在W3S0场景中，固定产量目标为56吨/天，优化目标为最小化储能的日折旧成本：
	\begin{equation}
		\min \; \mathcal{C}_{\text{storage}} = \frac{C_{\text{bat}} \cdot \gamma_{\text{inv}} \cdot 1000}{T_{\text{life}} \cdot 365} + P_{\text{bat}} \cdot \gamma_{\text{OM}}
		\label{eq:q4_obj}
	\end{equation}
	
	其中$\gamma_{\text{inv}}=1000$元/kWh为储能投资成本，$T_{\text{life}}=15$年为寿命，$\gamma_{\text{OM}}=0.01$元/kWh为运维费率。目标函数第一项将储能投资按直线折旧分摊至每日，第二项为按额定功率计算的运维费用。
	
	\textbf{求解结果。}以W3S0场景56吨/天为目标产量，求解上述LP模型得到最优储能配置为容量186.36 MWh、额定功率34.2 MW，储能日折旧成本34039元。该配置满足4C倍率约束，且使日弃电量从无储能时的173.7 MWh降至151.7 MWh。
	
	图\ref{fig:q4_offgrid_comparison}展示了24种场景下三种模式的日产量分布。离网有储能与无储能的日产量序列曲线基本重合，说明储能在当前配置下主要改善供电连续性，而对提升日产量的贡献有限。产量瓶颈主要来自低风光时段的新能源总量不足，日内储能只能将午间弃电转移至晚间，无法跨天气尺度弥补总量缺口。
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.88\textwidth]{q4_offgrid_comparison.pdf}
	...
	绿电直连型电氢氨园区作为高比例可再生能源消纳的"负荷侧柔性资源"，其规模化接入将显著改变配电网乃至主网的运行特性。本节从\textbf{系统级影响}出发，结合前四问的量化结果（尤其是Q2/Q3/Q4中24场景$\times$5产量$\times$两类运行模式的120$+$算例），系统分析园区容量渗透率提升（即园区用电功率占区域负荷比例上升）对电力系统的多维度影响，并提出可操作的政策建议。
	...
	\begin{table}[ht]
		...
		\begin{tabular}{p{28mm}p{48mm}p{60mm}}
			\toprule
			影响维度 & 机制说明 & 定量依据（源自前文结果） \\
			\midrule
			\textbf{(1) 新能源就地消纳能力提升} & 园区柔性负荷可动态跟踪风光出力，减少弃风弃光 & Q3连续调节下，\textbf{全满足绿电指标场景占比达70.83\%}（Q2仅39.17\%）；相较Q1基准（仅69.2\%绿电比例且$\eta_1$=28.2\%不合格），\textbf{Q3中全不满足场景完全消除}；W3S0场景弃电从177.3 MWh（无储能离网）降至151.7 MWh（配储能后），验证储能$+$柔性负荷协同消纳潜力。 \\
			\midrule
			\textbf{(2) 电网调峰压力缓解} & 园区在电价低谷时段主动降负荷，在高峰时段提升负荷，实现"削峰填谷" & Q2模型中，典型日W0S0场景下购电时段集中于$0\sim6$时（低价谷电），售电时段集中于$10\sim14$时（光伏峰值）；Q3连续调节进一步优化了这一调度模式，使园区净负荷波动幅度降低——全年24场景统计显示，相较于Q1固定运行模式，连续调节模式下绿电全满足场景由0个增至85个，系统的"源荷匹配"能力显著提升。 \\
			\midrule
			\textbf{(3) 系统惯量与频率支撑潜力} & 电解槽与合成氨装置具备毫秒级功率调节能力（文献\cite{1}），可参与一次调频 & Q3中合成氨爬坡约束设为20\%（0.3 MW/h），且PEM电解槽利用率达95\%，表明柔性制氢氨装置在多数时段处于可调度状态，具备为电网提供频率响应服务的潜力（参考IEEE Std 1547-2018对分布式资源的调频能力要求）。 \\
			\bottomrule
		\end{tabular}
	\end{table}
	...
	\begin{table}[ht]
		...
		\begin{tabular}{p{28mm}p{48mm}p{60mm}}
			...
			风险维度 & 问题本质 & 量化佐证（前文推导） \\
			\midrule
			\textbf{(1) 配电网电压越限风险加剧} & 高渗透率下，午间光伏大发$+$园区高负荷（如72吨/日）导致线路反向潮流，可能引发母线电压越上限 & Q1中典型日$12\sim14$时，$P_{\text{sell}}(t)$峰值达36.6 MW，若接入点短路容量仅100 MVA，反向潮流可能导致母线电压越上限（国标限值$\pm5\%$）；Q3连续调节可降低上网功率峰值，但仍存在局部时段电压越限风险。 \\
			\midrule
			...
			\midrule
			\textbf{(3) 离网自治可行性受限} & 储能仅能时移能量，无法弥补跨天气尺度的资源总量缺口 & Q4中，离网有储能年产量9512吨，仅为并网连续调节模式（12960吨）的\textbf{73.4\%}；即使配置186.36 MWh储能（投资约1.86亿元），\textbf{年产能缺口仍达3448吨（26.6\%）}，表明高渗透率园区若脱离电网，需依赖跨区互联或季节性储能（如氢储），当前技术经济性不足。 \\
			\bottomrule
		\end{tabular}
	\end{table}
	...
	\textbf{(1) 建立"绿电直连园区"专项容量电价机制}
	
	\par\quad 问题：当前分时电价未体现园区对电网调峰的正外部性，导致其经济性弱于传统负荷。
	
	\par\quad 建议：对绿电直连园区征收\textbf{容量备用费}（单位：元/kW$\cdot$月），费率与园区对系统调峰贡献挂钩。
	
	\par\quad 定量依据：Q3连续调节模式下，园区购电时段集中于电价低谷（$0\sim6$时），售电集中于光伏峰值（$10\sim14$时），实现了有效的"削峰填谷"；全年加权平均吨氨成本较Q2降低2.20\%，体现了柔性调度对系统运行经济性的正向贡献。建议容量费设定参照调峰服务市场均价，用于补偿电网侧灵活性资源投资。
	
	\textbf{(2) 推行"储能配置与绿电指标挂钩"的激励政策}
	
	\par\quad 问题：园区储能投资回收周期长（Q4中186 MWh储能日折旧34039元，纯折旧回收期约15年），企业积极性不足。
	
	\par\quad 建议：对满足"三项绿电指标全达标"的园区，给予\textbf{储能投资$30\%\sim50\%$补贴}，并允许其绿电消纳量折算为绿证额度。
	
	\par\quad 定量依据：Q3中全满足场景占比70.83\%，若补贴50\%，则186 MWh储能投资（1.86亿元）可降为0.93亿元，回收期缩短至约7.5年，显著提升经济可行性；同时，储能可提升绿电自用率（Q4对比数据），形成"合规$\to$补贴$\to$更多储能$\to$更高合规率"正反馈。
	
	\textbf{(3) 构建"绿证-碳市场-电价"三位一体联动机制}
	
	\par\quad 问题：当前绿电指标惩罚系数（$\lambda_1=0.5$, $\lambda_2=0.3$, $\lambda_3=1.5$ 元/kWh）偏低，企业为降成本容忍指标不达标。
	
	\par\quad 建议：将绿电指标不达标惩罚与全国碳市场配额价格动态挂钩，例如：$\lambda_{\text{green}} = 0.8 \times P_{\text{carbon}}$（$P_{\text{carbon}}$为碳价，单位元/吨CO$_2$），并允许园区用绿证抵消部分碳排放。
	
	\par\quad 定量依据：当前惩罚系数设定下，Q2中全不满足场景仍有8个（占6.67\%），表明现行惩罚力度不足以完全消除不合规行为。建议参考全国碳市场价格动态调整惩罚系数，并与绿证市场价格联动，形成动态调节机制，使合规的经济激励与实际减碳价值匹配。
	
	\textbf{(4) 完善电网备用服务定价，体现系统支撑价值}
	
	\par\quad 问题：园区柔性调节能力未被电网采购，导致"社会收益企业承担"。
	
	\par\quad 建议：将园区纳入\textbf{需求响应聚合商}体系，按其提供的调节容量（kW）和调节速率（kW/min）支付服务费。
	
	\par\quad 机制设计：参考广东现货市场规则，设定基础费率 $R = 15$ 元/kW$\cdot$月 $+$ $2.5$ 元/kW$\cdot$min$\cdot$次（针对爬坡响应）；Q3中设备总并网功率为41.5 MW，柔性调节能力可转化为可观的辅助服务收益，为园区提供额外收入来源以改善经济性。
	...
	为评估模型鲁棒性及关键参数影响程度，采用\textbf{单因素扰动法（One-At-A-Time, OAT）}，覆盖问题一至问题四全部模型，均在$\pm30\%$范围内取5个等距扰动点进行系统性测试。
	...
	设备容量参数影响最大（PEM 47.0\%、ALK 43.1\%），原因是容量变化同时改变设备折旧费（最大成本项）和购售电量，产生双重传导效应。分时电价影响中等（18.8\%），风光LCOE和运维费影响相对较小。
	...
	Q3连续调节模型\textbf{显著降低了参数敏感性}（比Q2降低12\%--20\%），可通过部分负荷运行缓冲外生参数变化。NH$_3$最低负荷率即使设到50\%也不影响最优解——最优调度中NH$_3$装置本就在高负荷运行。制氢效率参数影响具有场景依赖性：风光充裕时效率优势可释放，风光不足时受限于总可用能量。\textbf{PEM优先效应}得到验证：优化结果中PEM利用率约95\%，ALK仅约53\%，模型自动选择更高效的设备。
	...
	\vspace{0.3em}
	\noindent\textbf{关键发现1：}设备容量参数影响最大（OAT敏感度达47.0\%），与摘要中"影响程度达47\%"一致。该结论在Q1（解析计算）中最为显著——容量变化同时影响设备折旧和购售电量。
	
	\noindent\textbf{关键发现2：}绿电惩罚系数$\lambda_1$/$\lambda_2$/$\lambda_3$在Q2离散模型中\textbf{完全无影响}（变化幅度0\%），在Q3连续调节模型中影响有限。这一差异的根本原因在于：离散模型运行小时固定，调度方案不随惩罚系数变化；连续调节模型可通过调整功率分配部分吸收惩罚系数变化。
	...
	\begin{enumerate}
		\item \textbf{规划阶段应优先保证设备容量配置合理性}，避免"大马拉小车"；建议按风光资源禀赋动态调整ALK/PEM配比（高风速区侧重ALK，高辐照区侧重PEM）；
		\item \textbf{绿电指标考核应侧重"过程合规"而非仅结果惩罚}：Q2中惩罚系数完全不影响合规率的事实表明，离散运行模式下单纯提高惩罚力度无法改善合规水平，必须配套柔性调节手段；
		\item \textbf{政策制定需区分参数敏感性层级}：对高敏感参数（如设备容量）提供规划指导，对低敏感参数赋予企业自主调节空间。
	\end{enumerate}
	
	\subsection*{5.7 模型讨论}
	
	本文模型在以下方面做了简化处理。调度周期为单日，未考虑跨日储能累积过程——当风光连续多日不足时，储能不能在日内完成充放循环，日间调度结果可能低估储能的实际需求。设备启停采用理想化假设，未进一步考虑启停损耗与动态响应时间，对频繁启停场景的成本估计偏低。线性扩容假设未计入设备投资的规模经济效应，大容量方案的实际投资可能低于线性估算值。
